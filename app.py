...
st.set_page_config(page_title="El Chipo Pronos – IA 1X2 (Auto + API)", layout="wide")
st.title("🔥 El Chipo Pronos – Copilote IA (1X2) • MAJ auto quotidienne + Source API (gratuite)")
st.caption("Choisis: Upload CSV, URL CSV distante, ou **API gratuite** (football-data.org + The Odds API). Cache 24h + bouton Rafraîchir.")
...
with st.sidebar.expander("🔑 Clés API gratuites"):
    ...
    comps = st.text_input("Competitions football-data (IDs, ex: 2015=Ligue1,2021=PremierLeague)", "2015,2021")
...
if src_mode == "Upload CSV":
    ...
    if default_url:
        try:
            ...
            st.caption("✅ Fichier chargé depuis l'URL distante (cache 24h).")
    ...
    if df.empty:
        ...
        st.stop()
    st.success("✅ Données chargées via API (cache 24h).")
...
def format_vb(scored, side):
    ...
    nice["Proba Marché"] = nice["Proba Marché"].apply(lambda x: (x*100).round(1).astype(str) + "%" if pd.notna(x) else "—")
...
with st.expander("ℹ️ Détails tech & quotas"):
    st.markdown("""
- **football-data.org (gratuit)**: calendriers/résultats. Les cotes sont un add-on payant → on utilise **The Odds API (gratuit)** pour des cotes H2H.
- **Cache 24h**: les appels API sont mis en cache. Bouton **Rafraîchir** pour forcer une mise à jour.
- Si les cotes ne sont pas renvoyées (quota épuisé), l'app calcule quand même les **probas IA** et propose des opportunités (EV = `NaN`).
    """)
