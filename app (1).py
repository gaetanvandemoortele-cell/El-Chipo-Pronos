...
st.set_page_config(page_title="El Chipo Pronos – IA 1X2", layout="wide")
st.title("🔥 El Chipo Pronos – Copilote IA pour les Value Bets (1X2)")
st.caption("Analyse IA des matchs (aucune donnée fictive) • Probabilités H/D/A • EV • Mise Kelly capée")
...
with st.expander("ℹ️ Détails techniques"):
    st.markdown("""
- Modèle : **régression logistique multinomiale** (1X2) sur deux features simples (Δ rating, Δ forme).
- Marché : conversion cotes → probabilités **sans marge**.
- Sélections disponibles : **Domicile, Nul, Extérieur**.
- Mise : **Kelly fractionné** + plafonnement (% bankroll).
- 🇫🇷 *El Chipo Pronos* — démo éducative. Joue de manière responsable.
    """)
